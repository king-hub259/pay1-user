...
class EducationFeeHistoryScreen extends StatefulWidget {
  const EducationFeeHistoryScreen({super.key});

  @override
  State<EducationFeeHistoryScreen> createState() => _EducationFeeHistoryScreenState();
}

class _EducationFeeHistoryScreenState extends State<EducationFeeHistoryScreen> {
  final ScrollController historyScrollController = ScrollController();
  void fetchData() {
    Get.find<EducationFeeController>().getEducationHistoryDataList(
      forceLoad: false,
    );
  }
  ...
  void scrollListener() {
    if (historyScrollController.position.pixels == historyScrollController.position.maxScrollExtent) {
      if (Get.find<EducationFeeController>().hasNext()) {
        fetchData();
      }
    }
  }
    ...
    return GetBuilder<EducationFeeController>(
      builder: (controller) {
        return MyCustomScaffold(
          pageTitle: MyStrings.educationFeeHistory,
          body: CustomAppCard(
            child: Column(
              ...
              children: [
                Flexible(
                  ...
                  child: controller.isHistoryLoading
                      ...
                      : (controller.educationHistoryList.isEmpty)
                          ...
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemCount: controller.educationHistoryList.length + 1,
                                itemBuilder: (context, index) {
                                  if (controller.educationHistoryList.length == index) {
                                    return controller.hasNext() ? const CustomLoader(isPagination: true) : const SizedBox();
                                  }
                                  var item = controller.educationHistoryList[index];
                                  bool isLastIndex = index == controller.educationHistoryList.length - 1;
                                  return CustomCompanyListTileCard(
                                    imagePath: item.institution?.getInstituteImageUrl(),
  ...
  Widget buildDetailsSectionSheet({
    required LatestEducationFeeHistory item,
        ...
        children: [
          ...
          CustomCompanyListTileCard(
            leading: MyNetworkImageWidget(
              ...
              isProfile: true,
              imageUrl: item.institution?.getInstituteImageUrl() ?? "",
              imageAlt: item.institution?.name ?? "",
            ),
            showBorder: false,
            imagePath: item.institution?.getInstituteImageUrl(),
            title: item.institution?.name ?? "",
